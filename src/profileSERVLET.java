...
public class profileSERVLET extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			rs = db.getPhotos("select distinct p.photoid, p.username, p.title, p.description from photos p, allowlist a, tags t where p.photoid=a.photoid and a.photoid=t.photoid and a.username='"+session.getAttribute("username")+"' and p.username='"+usern+"' order by 1 desc");
			while(rs.next()){
			...
			rs = db.getPhotos("select photoid, username, title, description from photos where username='"+usern+"' and privacy='public' order by 1 desc");
			while(rs.next()){
				id.add(rs.getString(1));
				u.add(rs.getString(2));
				t.add(rs.getString(3));
