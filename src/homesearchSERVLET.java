...
public class homesearchSERVLET extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			rs = db.getPhotos("select distinct * from photos p, allowlist a, tags t where p.privacy='private' and p.photoid=a.photoid and a.photoid=t.photoid and t.tagname='"+tagname+"' and a.username='"+ssuname+"' order by 1 desc");
			while(rs.next()){
				if(!id.contains(rs.getString(1)))
					id.add(rs.getString(1));
				u.add(rs.getString(2));
				t.add(rs.getString(3));
				d.add(rs.getString(4));
			}
			rs = db.getPhotos("select distinct * from photos p, tags t where p.username='"+ssuname+"' and p.photoid=t.photoid and t.tagname='"+tagname+"' order by 1 desc");
			while(rs.next()){
				if(!id.contains(rs.getString(1)))
					id.add(rs.getString(1));
				u.add(rs.getString(2));
